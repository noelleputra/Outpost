...
public class NullaeOutpost extends JavaPlugin {
    ...
    private boolean luckPermsSupportEnabled = true;
    private LuckPerms luckPerms;

    private boolean placeholderAPISupportEnabled = true;

    // no toggle/on/off list
    public static Set<UUID> toggleList = new HashSet<>();

    /* ~~~~~~~~~~ Instance methods ~~~~~~~~~~~~ */

    /**
     * Add a command argument to /no.
     *
     * @param psca NOCommandArg object to be added
     */
    public void addCommandArgument(NOCommandArg psca) {
        commandArgs.add(psca);
    }

    /**
     * @return the list of command arguments for /no
     */
    public List<NOCommandArg> getCommandArguments() {
        return commandArgs;
    }
    ...
    /**
     * Returns the list of NOProtectBlocks configured through the config.
     *
    ...
    public static boolean isNONameAlreadyUsed(String name) {
     ...
     * Note: Does not remove the NO block.
     *
     ...
     * Note: Does not remove the NO block, and does not check if the player (cause) has permission to do this.
     *
     * @param w     the world that the region is in
     * @param psID  the worldguard region ID of the region
    ...
    public static ItemStack createProtectBlockItem(NOProtectBlock b) {
        ...
        im.getCustomTagContainer().setCustomTag(new NamespacedKey(plugin, "isNOBlock"), ItemTagType.BYTE, (byte) 1);
    ...
    // called on first start, and /no reload
    public static void loadConfig(boolean isReload) {
        // remove old no crafting recipes
    ...
    public void onEnable() {
        ...
        for (World w : regionManagers.keySet()) {
            ...
            for (ProtectedRegion r : rgm.getRegions().values()) {
                String name = r.getFlag(FlagHandler.NO_NAME);
                if (isNORegion(r) && name != null) {
        ...
        getLogger().info("Checking if NO regions have been updated to UUIDs...");

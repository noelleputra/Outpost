...
public class WGUtils {
    ...
    public static Set<ProtectedRegion> findOverlapOrAdjacentRegions(ProtectedRegion r, RegionManager rgm, World w) {
        ...
        // We have a major issue with detecting adjacent regions when the other regions are NOGroupRegion (the one adjacent
    ...
    public static String matchLocationToNOID(Location l) {
            ...
            for (String currentID : idList) {
                if (NullaeOutpost.isNORegionFormat(rgm.getRegion(currentID))) {
                    ...
                    if (distanceToNO == -1 || tempToNO < distanceToNO) {
                        distanceToNO = tempToNO;
                        currentNOID = currentID;
                    }
                }
            }
        }
    ...
    // create NO ids without making the numbers have scientific notation (addressed with long)
    public static String createNOID(double bx, double by, double bz) {
        return "no" + (long) bx + "x" + (long) by + "y" + (long) bz + "z";
    }

    public static String createNOID(Location l) {
        return createNOID(l.getX(), l.getY(), l.getZ());
    }

...
public class LegacyUpgrade {
            ...
            for (ProtectedRegion r : rm.getRegions().values()) {
                if (NullaeOutpost.isNORegion(r)) {
                    ...
                    if (psr instanceof NOGroupRegion) {
                        ...
                        for (NOMergedRegion psmr : psgr.getMergedRegions()) {
                            ...
                            if (oldToNew.containsKey(type)) {
                                Set<String> flag = psmr.getGroupRegion().getWGRegion().getFlag(FlagHandler.NO_MERGED_REGIONS_TYPES);
                    ...
                    if (oldToNew.containsKey(psr.getType())) {
                        psr.getWGRegion().setFlag(FlagHandler.NO_BLOCK_MATERIAL, oldToNew.get(psr.getType()));
                    }
    ...
    // check that all of the NO custom flags are in no regions and upgrade if not
            ...
            for (String regionName : rm.getRegions().keySet()) {
                if (regionName.startsWith("no") && !NullaeOutpost.isNORegion(rm.getRegion(regionName))) {
                    try {
                        ...
                        if (r.getFlag(FlagHandler.NO_BLOCK_MATERIAL) == null) {
                            r.setFlag(FlagHandler.NO_BLOCK_MATERIAL, material);
                        }
    ...
    public static void convertToUUID() {
        Bukkit.getLogger().info("Updating NO regions to UUIDs...");
        for (World world : Bukkit.getWorlds()) {
            RegionManager rm = WGUtils.getRegionManagerWithWorld(world);

            // iterate over regions in world
            for (String regionName : rm.getRegions().keySet()) {
                if (regionName.startsWith("no")) {
    ...
    public static void upgradeFromV1V2() {
        ...
        Bukkit.getLogger().info(ChatColor.AQUA + "Updating NO Regions to new format...");

...
public class ArgGet implements NOCommandArg {
    ...
    private boolean openGetGUI(Player p) {
        ...
        for (OutpostProtectBlock b : Outpost.getInstance().getConfiguredBlocks()) {
            ...
            TextComponent tc = new TextComponent(OutpostL.GET_GUI_BLOCK.msg()
            ...
            tc.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(OutpostL.GET_GUI_HOVER.msg()
                    .replace("%alias%", b.alias)
                    .replace("%price%", price)
                    .replace("%description%", b.description)
                    .replace("%xradius%", ""+b.xRadius)
            ...
            tc.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/" + Outpost.getInstance().getConfigOptions().base_command + " get " + b.alias));
    ...
    public boolean executeArgument(CommandSender s, String[] args, HashMap<String, String> flags) {
        ...
        if (args.length != 2)
            return OutpostL.msg(p, OutpostL.GET_HELP.msg());
        ...
        if (!p.getInventory().addItem(cp.createItem()).isEmpty()) {
            if (Outpost.getInstance().getConfigOptions().dropItemWhenInventoryFull) { // drop on floor
        ...
        return OutpostL.msg(p, OutpostL.GET_GOTTEN.msg());
    }

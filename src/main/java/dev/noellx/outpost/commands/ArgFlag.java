...
public class ArgFlag implements NOCommandArg {
    ...
    private static final List<String> FLAG_GROUNO = FlagHandler.FLAG_GROUNO;
    ...
    private boolean openFlagGUI(Player p, OutpostRegion r, int page) {
        ...
        for (int i = GUI_SIZE * page; i < Math.min(allowedFlags.size(), GUI_SIZE * page + GUI_SIZE); i++) {
            if (i >= allowedFlags.size()) {
                ...
                // send message
                flagLine.addExtra(getDots(40 - REGION_GROUP_KERNING_LENGTHS[FLAG_GROUNO.indexOf(groupfValue)]) + ChatColor.AQUA + " " + flag);
    ...
    public List<String> tabComplete(CommandSender sender, String alias, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 2) { // -g, or allowed flag names
                ...
                keywords.addAll(FlagHandler.FLAG_GROUNO);
